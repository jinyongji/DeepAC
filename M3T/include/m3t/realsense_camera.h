...
class RealSense {
 public:
  ...
  void UseColorCamera();
  void UseDepthCamera();
  void UseIRCamera(int ir_index = 1);  // ir_index: 1 for left IR, 2 for right IR
  void SetEmitterEnabled(bool enabled);  // Control structured light emitter
  int RegisterID();
  ...
  bool use_ir_camera() const;
  int ir_camera_index() const;
 ...
 private:
  ...
  bool use_ir_camera_ = false;
  int ir_camera_index_ = 1;  // Default to left IR (index 1)
  bool emitter_enabled_ = true;  // Default to enabled (structured light on)
...
/**
 * \brief \ref Camera that allows getting IR (infrared) images from a \ref RealSense
 * camera.
 *
 * @param ir_index specifies which IR camera to use (1 for left IR, 2 for right IR).
 * @param use_color_as_world_frame specifies the color camera frame as world
 * frame and automatically defines `camera2world_pose` as `ir2color_pose`.
 */
class RealSenseIRCamera : public ColorCamera {
 public:
  // Constructors, destructor, and setup method
  RealSenseIRCamera(const std::string &name,
                    int ir_index = 1,
                    bool use_color_as_world_frame = true);
  RealSenseIRCamera(const std::string &name,
                    const std::filesystem::path &metafile_path);
  RealSenseIRCamera(const RealSenseIRCamera &) = delete;
  RealSenseIRCamera &operator=(const RealSenseIRCamera &) = delete;
  ~RealSenseIRCamera();
  bool SetUp() override;

  // Setters
  void set_ir_index(int ir_index);
  ...
  const Transform3fA *color2ir_pose() const;
  const Transform3fA *ir2color_pose() const;
