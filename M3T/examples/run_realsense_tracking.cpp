...
int main(int argc, char *argv[]) {
  ...
  // IR相机（左IR，索引为1）- 使用颜色相机作为世界坐标系
  auto ir_camera_ptr{
      std::make_shared<m3t::RealSenseIRCamera>("realsense_ir", 1, true)};  // ir_index=1 (left IR), use_color_as_world_frame=true
  
  // 关闭Emitter（结构光发射器），这样IR图像中就不会有光点
  // 这比在图像处理层面去除光点更高效
  m3t::RealSense::GetInstance().SetEmitterEnabled(false);
  
  // 设置 viewer（颜色、深度和IR视图）
  ...
  auto ir_viewer_ptr{std::make_shared<m3t::NormalColorViewer>(
  ...
  std::cout << "  - Images: combined_image_*.png (RGB + Depth + IR side by side)" << std::endl;
  std::cout << "  - Video: tracking_video.mp4 (generated automatically)" << std::endl;
  std::cout << "  - Poses: pose.txt" << std::endl;
  std::cout << "========================================" << std::endl;
  ...
  ir_viewer_ptr->StartSavingImages(output_directory, "png");
  ...
  while (tracking) {
    ...
    std::string ir_filename = "ir_viewer_image_" + std::to_string(iteration) + ".png";
    ...
    std::filesystem::path ir_image_path = output_directory / ir_filename;
    
    cv::Mat color_image = cv::imread(color_image_path.string());
    cv::Mat depth_image = cv::imread(depth_image_path.string());
    cv::Mat ir_image = cv::imread(ir_image_path.string());
    
    if (!color_image.empty() && !depth_image.empty() && !ir_image.empty()) {
      // 调整图像大小使其一致（如果大小不同）
      cv::Size target_size = color_image.size();
      if (depth_image.size() != target_size) {
        cv::resize(depth_image, depth_image, target_size);
      }
      if (ir_image.size() != target_size) {
        cv::resize(ir_image, ir_image, target_size);
      }
      
      // 横向拼接图像（RGB在左，深度在中，IR在右）
      cv::Mat combined_image;
      cv::Mat temp_combined;
      cv::hconcat(color_image, depth_image, temp_combined);
      cv::hconcat(temp_combined, ir_image, combined_image);
  ...
  std::cout << "  - Poses: " << pose_file_path << std::endl;
  std::cout << "  - Images: " << output_directory << "/combined_image_*.png (RGB + Depth + IR)" << std::endl;
