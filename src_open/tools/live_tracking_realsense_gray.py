...
def parse_args():
    parser = argparse.ArgumentParser(description="DeepAC live tracking runner for RealSense IR camera")
    parser.add_argument(
        "--cfg",
        type=str,
        required=True,
...
def preprocess_frame(frame_bgr, camera_cfg, device):
    """构建原始相机，不进行resize
    对于IR图像，frame_bgr可能是单通道灰度图，需要转换为RGB
    """
    # 如果是单通道图像，转换为3通道RGB（通过复制通道）
...
def tensor_bbox_to_xyxy(bbox_tensor):
    """将bbox张量转换为(x1, y1, x2, y2)格式"""
    if bbox_tensor.ndim > 1:
        bbox_tensor = bbox_tensor[0]
    cx, cy, w, h = bbox_tensor[:4].cpu().numpy()
    x1 = cx - w / 2
    y1 = cy - h / 2
    x2 = cx + w / 2
    y2 = cy + h / 2
...
def bbox_iou(bbox_a, bbox_b):
    ...
    ax1, ay1, ax2, ay2 = tensor_bbox_to_xyxy(bbox_a)
    bx1, by1, bx2, by2 = tensor_bbox_to_xyxy(bbox_b)
    ...
    if centers_in_image is not None and centers_valid is not None:
        try:
            if centers_in_image.ndim == 3:
                centers_flat = centers_in_image[0]
            elif centers_in_image.ndim == 2:
                centers_flat = centers_in_image
            else:
                centers_flat = None
            ...
            if centers_valid.ndim == 3:
                ...
                if centers_valid.shape[0] == 1:
                    valid_flat = centers_valid[0]
                else:
                    valid_flat = centers_valid[0]
            elif centers_valid.ndim == 1:
                valid_flat = centers_valid
            else:
                valid_flat = None
...
def main():
    ...
    logger = MyLightningLogger("deepac-live-realsense-ir", str(save_dir))
    ...
    # 初始化RealSense IR相机
    ir_index = int(cfg.camera.get("ir_index", 1))
    emitter_enabled = bool(cfg.camera.get("emitter_enabled", False))
    realsense_camera = M3TRealSenseIRCamera(ir_index=ir_index, emitter_enabled=emitter_enabled)
    
    width = int(cfg.camera.get("set_width", 1280))
    height = int(cfg.camera.get("set_height", 720))
    fps = int(cfg.camera.get("fps", 30))
    
    if not realsense_camera.setup(width=width, height=height, fps=fps):
        raise RuntimeError("Failed to setup RealSense IR camera")
    
    intrinsics = realsense_camera.get_intrinsics()
    if intrinsics is None:
        raise RuntimeError("Failed to get RealSense IR camera intrinsics")
    
    # 更新配置中的内参（使用实际获取的内参）
    cfg.camera.fx = intrinsics["fx"]
    cfg.camera.fy = intrinsics["fy"]
    cfg.camera.cx = intrinsics["cx"]
    cfg.camera.cy = intrinsics["cy"]
    
    logger.info(
        f"RealSense IR camera initialized: {intrinsics['width']}x{intrinsics['height']}, "
    ...
    if cfg.tracking.output_video:
        ...
        video_path = save_dir / f"live_realsense_ir_{time.strftime('%Y%m%d_%H%M%S')}.avi"
    ...
    # 创建CLAHE对象用于增强IR图像对比度
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
    use_clahe = cfg.tracking.get("use_clahe", True)  # 默认启用CLAHE增强
    ...
    try:
        while True:
            ...
            # 对IR图像进行CLAHE增强以提高对比度
            if use_clahe:
                ir_image = clahe.apply(ir_image)

            # 将单通道IR图像转换为BGR格式（用于显示和处理）
            ...
            if cfg.tracking.show_window:
                cv2.imshow("DeepAC Live (RealSense IR)", overlay)
